...
data class RemoteCommit(val index: Long, val spec: CommitmentSpec, val txid: TxId, val remotePerCommitmentPoint: PublicKey) {
    ...
    fun sign(channelParams: ChannelParams, channelKeys: ChannelKeys, signingSession: InteractiveTxSigningSession, remoteNonce: IndividualNonce?, logger: MDCLogger): Either<ChannelException, CommitSig> {
        return sign(
            ...
            channelKeys,
            signingSession.fundingParams.fundingTxIndex,
...
data class Commitment(
    ...
    fun sendCommit(
        ...
        val sig = when (commitmentFormat) {
            Transactions.CommitmentFormat.AnchorOutputs -> remoteCommitTx.sign(fundingKey, remoteFundingPubkey)
            Transactions.CommitmentFormat.SimpleTaprootChannels -> when (nextRemoteNonce) {
...
data class Commitments(
    ...
    fun receiveCommit(commits: CommitSigs, channelKeys: ChannelKeys, logger: MDCLogger): Either<ChannelException, Pair<Commitments, RevokeAndAck>> {
        ...
        val localCommitNonces = active.mapNotNull { c ->
            when (c.commitmentFormat) {
                ...
                Transactions.CommitmentFormat.SimpleTaprootChannels -> {
                    ...
                    c.fundingTxId to localNonce.publicNonce
                }
            }
        }
        val revocation = RevokeAndAck(channelId, localPerCommitmentSecret, localNextPerCommitmentPoint, localCommitNonces)
        val commitments1 = copy(
            active = active1,
            changes = changes.copy(
        ...
        updateFundingStatus(fundingTx.txid) { c: Commitment, _: Long ->
            if (c.fundingTxId == fundingTx.txid) {
                ...
                when (c.localFundingStatus) {
                    ...
                    is LocalFundingStatus.ConfirmedFundingTx -> when {
                        c.localFundingStatus.shortChannelId == ShortChannelId(0) || c.localFundingStatus.shortChannelId.blockHeight() == 0 -> {
                            logger.info { "updating short_channel_id for fundingTxId=${fundingTx.txid} from ${c.localFundingStatus.shortChannelId} to $shortChannelId" }
