    ...
    override suspend fun ChannelContext.processInternal(cmd: ChannelCommand): Pair<ChannelState, List<ChannelAction>> {
        ...
        return when (cmd) {
            ...
            is ChannelCommand.Close.MutualClose -> {
                val allowAnySegwit = Features.canUseFeature(commitments.channelParams.localParams.features, commitments.channelParams.remoteParams.features, Feature.ShutdownAnySegwit)
                val allowOpReturn = Features.canUseFeature(commitments.channelParams.localParams.features, commitments.channelParams.remoteParams.features, Feature.SimpleClose)
                val localScriptPubkey = cmd.scriptPubKey ?: commitments.channelParams.localParams.defaultFinalScriptPubKey
                logger.warning { "[260507-DBG] CMD MutualClose RECEIVED: scriptPubKey=${localScriptPubkey.toHex()} localShutdown=${localShutdown != null} unsignedOut=${commitments.changes.localHasUnsignedOutgoingHtlcs()} unsignedFee=${commitments.changes.localHasUnsignedOutgoingUpdateFee()} validScript=${Helpers.Closing.isValidFinalScriptPubkey(localScriptPubkey, allowAnySegwit, allowOpReturn)}" }
                when {
                    localShutdown != null -> {
            ...
            is ChannelCommand.Close.ForceClose -> {
                logger.warning { "[260507-DBG] CMD ForceClose RECEIVED in Normal state, going to local commit publish" }
                handleLocalError(cmd, ForcedLocalCommit(channelId))
            }
            ...
            is ChannelCommand.MessageReceived -> when {
                cmd.message is ForbiddenMessageDuringSplice && spliceStatus is QuiescentSpliceStatus -> {
                    ...
                    is Shutdown -> {
                        val allowAnySegwit = Features.canUseFeature(commitments.channelParams.localParams.features, commitments.channelParams.remoteParams.features, Feature.ShutdownAnySegwit)
                        val allowOpReturn = Features.canUseFeature(commitments.channelParams.localParams.features, commitments.channelParams.remoteParams.features, Feature.SimpleClose)
                        logger.warning { "[260507-DBG] Shutdown received: scriptPubKey=${cmd.message.scriptPubKey.toHex()} closeeNonce=${cmd.message.closeeNonce} allowAnySegwit=$allowAnySegwit allowOpReturn=$allowOpReturn localShutdown=${localShutdown != null} spliceStatus=${spliceStatus::class.simpleName} activeCount=${commitments.active.size}" }
                        ...
                        when {
                            !Helpers.Closing.isValidFinalScriptPubkey(cmd.message.scriptPubKey, allowAnySegwit, allowOpReturn) -> {
                                logger.warning { "[260507-DBG] Shutdown REJECTED: invalid scriptPubKey ${cmd.message.scriptPubKey.toHex()}" }
                                handleLocalError(cmd, InvalidFinalScript(channelId))
                            }
                            commitments.changes.remoteHasUnsignedOutgoingHtlcs() -> {
                                logger.warning { "[260507-DBG] Shutdown REJECTED: remote has unsigned outgoing HTLCs" }
                                handleLocalError(cmd, CannotCloseWithUnsignedOutgoingHtlcs(channelId))
                            }
                            commitments.changes.remoteHasUnsignedOutgoingUpdateFee() -> {
                                logger.warning { "[260507-DBG] Shutdown REJECTED: remote has unsigned outgoing UpdateFee" }
                                handleLocalError(cmd, CannotCloseWithUnsignedOutgoingUpdateFee(channelId))
                            }
                            commitments.changes.localHasUnsignedOutgoingHtlcs() -> {
                                logger.warning { "[260507-DBG] Shutdown branch: localHasUnsignedOutgoingHtlcs=true, going to sign" }
                                ...
                                logger.warning { "[260507-DBG] Shutdown clean branch entered: localShutdown=${localShutdown != null} closeCommand=${closeCommand != null} hasNoPendingHtlcs=${commitments.hasNoPendingHtlcsOrFeeUpdate()}" }
                                ...
                                if (localShutdown == null) {
                                    logger.warning { "[260507-DBG] sending our Shutdown reply scriptPubKey=${localShutdown1.scriptPubKey.toHex()}" }
                                    actions.add(ChannelAction.Message.Send(localShutdown1))
                                }
                                when {
                                    commitments.hasNoPendingHtlcsOrFeeUpdate() -> {
                                        logger.warning { "[260507-DBG] starting closing negotiation (NEGOTIATING_SIMPLE)" }
                                        startClosingNegotiation(
                                            closeCommand,
                                            commitments,
                                            localShutdown1,
                                            localCloseeNonce1,
                                        ...
                                        logger.warning { "[260507-DBG] going to ShuttingDown state (pending changes)" }
                                ...
                                if (!cmd.message.initiator || isChannelOpener) {
                                    if (commitments.isQuiescent()) {
                                        ...
                                        val commitTxFees = when {
                                            paysCommitTxFees -> Transactions.commitTxFee(commitments.latest.remoteCommitParams.dustLimit, parentCommitment.remoteCommit.spec, commitments.latest.commitmentFormat)
                                            else -> 0.sat
                                        }
                    ...
                    is SpliceAck -> when (spliceStatus) {
                        is SpliceStatus.Requested -> {
                            ...
                            when (val liquidityPurchase = LiquidityAds.validateRemoteFunding(
                                ...
                                remoteNodeId,
                                channelId,
                                Transactions.makeFundingScript(spliceStatus.spliceInit.fundingPubkey, cmd.message.fundingPubkey, commitments.latest.commitmentFormat).pubkeyScript,
