...
data class SharedFundingInput(
    val info: Transactions.InputInfo,
    val fundingTxIndex: Long,
    val remoteFundingPubkey: PublicKey,
    val commitmentFormat: Transactions.CommitmentFormat,
) {
    constructor(channelKeys: ChannelKeys, commitment: Commitment) : this(
    ...
    fun sign(session: InteractiveTxSession, tx: Transaction, spentUtxos: Map<OutPoint, TxOut>): Either<ChannelException, ChannelSpendSignature> {
        ...
        return when (commitmentFormat) {
            ...
            Transactions.CommitmentFormat.SimpleTaprootChannels -> {
                ...
                when (val psig = spliceTx.partialSign(fundingKey, remoteFundingPubkey, spentUtxos, localNonce, orderedNonces)) {
...
data class InteractiveTxParams(
    ...
    ) : this(channelId, isInitiator, localContribution, remoteContribution, null, remoteFundingPubKey, listOf(), commitmentFormat, lockTime, dustLimit, targetFeerate)
...
data class FundingContributions(val inputs: List<InteractiveTxInput.Outgoing>, val outputs: List<InteractiveTxOutput.Outgoing>) {
    companion object {
        ...
        fun computeSpliceContribution(
            channelKeys: ChannelKeys,
            isInitiator: Boolean,
            commitment: Commitment,
            walletInputs: List<WalletState.Utxo>,
            localOutputs: List<TxOut>,
            isLiquidityPurchase: Boolean,
            targetFeerate: FeeratePerKw
        ): Satoshi {
            val weight = computeWeightPaid(channelKeys, isInitiator, commitment, walletInputs, localOutputs)
        ...
        fun create(
            ...
            val changeOutput = when (changePubKey) {
                null -> listOf()
                    ...
                    val weightWithChange = computeWeightPaid(params.isInitiator, sharedUtxo?.first, fundingPubkeyScript, walletInputs, localOutputs + listOf(TxOut(0.sat, Script.pay2wpkh(changePubKey))))
        ...
        fun computeWeightPaid(channelKeys: ChannelKeys, isInitiator: Boolean, commitment: Commitment, walletInputs: List<WalletState.Utxo>, localOutputs: List<TxOut>): Int =
            computeWeightPaid(
                isInitiator,
                SharedFundingInput(channelKeys, commitment),
                commitment.commitInput(channelKeys).txOut.publicKeyScript,
                walletInputs,
                localOutputs
            )
...
data class SharedTransaction(
    ...
    fun sign(session: InteractiveTxSession, keyManager: KeyManager, fundingParams: InteractiveTxParams, remoteNodeId: PublicKey): Either<ChannelException, PartiallySignedSharedTransaction> {
        ...
        val txSigs = TxSignatures(fundingParams.channelId, unsignedTx, listOf(), sharedSig, legacySwapUserSigs, legacySwapServerSigs, swapUserPartialSigs, swapServerPartialSigs)
        return Either.Right(PartiallySignedSharedTransaction(this, txSigs))
    }
}
    ...
    fun addRemoteSigs(channelKeys: ChannelKeys, fundingParams: InteractiveTxParams, remoteSigs: TxSignatures): FullySignedSharedTransaction? {
        ...
        val sharedSigs = fundingParams.sharedInput?.let { input ->
            ...
            val signedTx = when (input.commitmentFormat) {
                ...
                Transactions.CommitmentFormat.SimpleTaprootChannels -> spliceTx.aggregateSigs(
                    localFundingPubkey,
                    input.remoteFundingPubkey,
...
data class InteractiveTxSession(
    ...
    val localFundingNonce: Transactions.LocalNonce? = null,
) {
    ...
    fun send(): Pair<InteractiveTxSession, InteractiveTxSessionAction> {
        return when (val msg = toSend.firstOrNull()) {
            null -> {
                ...
                val txComplete = when (fundingParams.commitmentFormat) {
                    Transactions.CommitmentFormat.AnchorOutputs -> TxComplete(fundingParams.channelId, TlvStream(TxCompleteTlv.SwapInNonces(swapInNonces)))
                    Transactions.CommitmentFormat.SimpleTaprootChannels -> {
                        ...
                        val fundingTxId = Transaction(
                            version = 2,
                            txIn = (localInputs.filterIsInstance<InteractiveTxInput>() + remoteInputs.filterIsInstance<InteractiveTxInput>())
                                .map { it.serialId to TxIn(it.outPoint, it.sequence.toLong()) }
                                .sortedBy { it.first }
                                .map { it.second },
                            txOut = (localOutputs.filterIsInstance<InteractiveTxOutput>() + remoteOutputs.filterIsInstance<InteractiveTxOutput>())
                                .map { it.serialId to TxOut(it.amount, it.pubkeyScript) }
                                .sortedBy { it.first }
                                .map { it.second },
                        ...
                        TxComplete(
                            ...
                            commitNonce = NonceGenerator.verificationNonce(fundingTxId, localFundingKey, fundingParams.remoteFundingPubkey, localCommitIndex).publicNonce,
                            nextCommitNonce = NonceGenerator.verificationNonce(fundingTxId, localFundingKey, fundingParams.remoteFundingPubkey, localCommitIndex + 1).publicNonce,
                            fundingNonce = localFundingNonce?.publicNonce,
...
data class InteractiveTxSigningSession(
    ...
    val nextRemoteCommitNonce: IndividualNonce?
) {
    //                      Example flow:
    //     +-------+                             +-------+
    //     |       |-------- commit_sig -------->|       |
    ...
    fun commitInput(fundingKey: PrivateKey): Transactions.InputInfo {
        ...
        val fundingOutput = OutPoint(fundingTx.txId, fundingTx.tx.buildUnsignedTx().txOut.indexOfFirst { it.amount == fundingParams.fundingAmount && it.publicKeyScript == fundingScript }.toLong())
        return Transactions.InputInfo(fundingOutput, TxOut(fundingParams.fundingAmount, fundingScript))
    }
    ...
    fun receiveCommitSig(channelKeys: ChannelKeys, channelParams: ChannelParams, remoteCommitSig: CommitSig, currentBlockHeight: Long, logger: MDCLogger): Pair<InteractiveTxSigningSession, InteractiveTxSigningSessionAction> {
        return when (localCommit) {
            is Either.Left -> {
                ...
                val fundingInput = commitInput(fundingKey)
                    ...
                    is Either.Right -> {
                        if (shouldSignFirst(fundingParams.isInitiator, channelParams, fundingTx.tx)) {
                            ...
                            val commitment = Commitment(
                                fundingParams.fundingTxIndex,
                                fundingInput.outPoint,
                                fundingParams.fundingAmount,
                                fundingParams.remoteFundingPubkey,
                                fundingStatus,
                                RemoteFundingStatus.NotLocked,
                                fundingParams.commitmentFormat,
                                localCommitParams,
                                signedLocalCommit.value,
                                remoteCommitParams,
                                remoteCommit,
                                nextRemoteCommit = null
                            )
                            val action = InteractiveTxSigningSessionAction.SendTxSigs(fundingStatus, commitment, fundingTx.localSigs, nextRemoteCommitNonce)
        ...
        return when (localCommit) {
            ...
            is Either.Right -> when (val fullySignedTx = fundingTx.addRemoteSigs(channelKeys, fundingParams, remoteTxSigs)) {
                null -> Either.Left(InteractiveTxSigningSessionAction.AbortFundingAttempt(InvalidFundingSignature(fundingParams.channelId, fundingTx.txId)))
                    ...
                    val fundingInput = commitInput(channelKeys)
                    ...
                    Either.Right(InteractiveTxSigningSessionAction.SendTxSigs(fundingStatus, commitment, fundingTx.localSigs, nextRemoteCommitNonce))
                }
            }
        }
    }
    ...
    companion object {
        ...
        data class UnsignedLocalCommit(val index: Long, val spec: CommitmentSpec, val txId: TxId)
        ...
        fun create(
            ...
            val remoteCommitKeys = session.channelKeys.remoteCommitmentKeys(channelParams, remotePerCommitmentPoint)
            val unsignedTx = sharedTx.buildUnsignedTx()
            val sharedOutputIndex = unsignedTx.txOut.indexOfFirst { it.publicKeyScript == fundingParams.fundingPubkeyScript(session.channelKeys) }.toLong()
            ...
            return Helpers.Funding.makeCommitTxs(
                ...
                val localSigOfRemoteCommitTx = when (fundingParams.commitmentFormat) {
                    Transactions.CommitmentFormat.AnchorOutputs -> firstCommitTx.remoteCommitTx.sign(fundingKey, fundingParams.remoteFundingPubkey)
                    Transactions.CommitmentFormat.SimpleTaprootChannels -> {
                        val remoteNonce = session.currentRemoteCommitNonce ?: return Either.Left(MissingCommitNonce(channelParams.channelId, unsignedTx.txid, remoteCommitmentIndex))
                        val localNonce = NonceGenerator.signingNonce(fundingKey.publicKey(), fundingParams.remoteFundingPubkey, unsignedTx.txid)
                        // DEV-BYPASS: publicNonces[i] must correspond to sortedKeys[i].
                        val sortedFundingKeys2 = Scripts.sort(listOf(fundingKey.publicKey(), fundingParams.remoteFundingPubkey))
                        val orderedNonces2 = if (sortedFundingKeys2.first() == fundingKey.publicKey())
                            listOf(localNonce.publicNonce, remoteNonce)
                        else
                            listOf(remoteNonce, localNonce.publicNonce)
                        when (val psig = firstCommitTx.remoteCommitTx.partialSign(fundingKey, fundingParams.remoteFundingPubkey, mapOf(), localNonce, orderedNonces2)) {
                            is Either.Left -> return Either.Left(InvalidCommitNonce(channelParams.channelId, unsignedTx.txid, remoteCommitmentIndex))
                ...
                sharedTx.sign(session, keyManager, fundingParams, channelParams.remoteParams.nodeId).map { signedFundingTx ->
