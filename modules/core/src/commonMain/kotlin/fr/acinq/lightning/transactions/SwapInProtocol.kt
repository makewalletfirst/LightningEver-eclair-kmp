...
data class SwapInProtocol(val userPublicKey: PublicKey, val serverPublicKey: PublicKey, val userRefundKey: PublicKey, val refundDelay: Int) {
    ...
    fun witness(fundingTx: Transaction, index: Int, parentTxOuts: List<TxOut>, userNonce: IndividualNonce, serverNonce: IndividualNonce, userPartialSig: ByteVector32, serverPartialSig: ByteVector32): Either<Throwable, ScriptWitness> {
        val sortedNonces = listOf(userPublicKey to userNonce, serverPublicKey to serverNonce)
            .sortedWith { a, b -> LexicographicalOrdering.compare(a.first.value, b.first.value) }
            .map { it.second }
        val sortedSigs = listOf(userPublicKey to userPartialSig, serverPublicKey to serverPartialSig)
            .sortedWith { a, b -> LexicographicalOrdering.compare(a.first.value, b.first.value) }
            .map { it.second }
        return Musig2.aggregateTaprootSignatures(sortedSigs, fundingTx, index, parentTxOuts, sortedKeys, sortedNonces, scriptTree).map { aggregateSig ->
