    ...
    override suspend fun ChannelContext.processInternal(cmd: ChannelCommand): Pair<ChannelState, List<ChannelAction>> {
        return when (cmd) {
            is ChannelCommand.MessageReceived -> when (cmd.message) {
                ...
                is ChannelReady -> {
                    if (staticParams.useZeroConf && latestFundingTx.sharedTx is FullySignedSharedTransaction) {
                        logger.info { "channel is using 0-conf and we received channel_ready, activating immediately" }
                        val signedTx = latestFundingTx.sharedTx.signedTx
                        val dummyTxIndex = kotlin.math.abs(fr.acinq.bitcoin.crypto.Pack.int32BE(signedTx.txid.value.slice(0, 16).toByteArray()))
                        
                        val res = commitments.run {
                            updateLocalFundingConfirmed(signedTx, 0, dummyTxIndex)
                        }
                        when (res) {
                            is Either.Left -> Pair(this@WaitForFundingConfirmed.copy(deferred = cmd.message), listOf())
                            is Either.Right -> {
                                val commitments1 = res.value.first
                                val commitment = res.value.second
                                val shortChannelId = (commitment.localFundingStatus as LocalFundingStatus.ConfirmedFundingTx).shortChannelId
                                val commitInput = commitment.commitInput(channelKeys())
                                val watchSpent = fr.acinq.lightning.blockchain.WatchSpent(channelId, commitment.fundingTxId, commitInput.outPoint.index.toInt(), commitInput.txOut.publicKeyScript, fr.acinq.lightning.blockchain.WatchSpent.ChannelSpent(commitment.fundingAmount))
                                val actions = buildList {
                                ...
                                val nextState = WaitForChannelReady(commitments1, remoteNextCommitNonces, shortChannelId, channelReady)
                                val actions1 = buildList {
                                    if (rbfStatus != RbfStatus.None) add(ChannelAction.Message.Send(TxAbort(channelId, InvalidRbfTxConfirmed(channelId, signedTx.txid).message)))
                                    add(ChannelAction.Message.Send(channelReady))
                                    add(ChannelAction.Storage.StoreState(nextState))
                                }
                                val (nextState1, actions2) = nextState.run { process(ChannelCommand.MessageReceived(cmd.message)) }
                                Pair(nextState1, actions + actions1 + actions2)
                            }
                        }
                    } else {
                        Pair(this@WaitForFundingConfirmed.copy(deferred = cmd.message), listOf())
                    }
                }
